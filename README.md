...
To clone and checkout the `develop` branch, run
```bash
git clone -b develop https://github.com/jaz-lang/jaz.git
cd jaz
```
...
uv sync --extra tracing             # + OpenTelemetry tracing
uv sync --extra eval-oolong         # + Oolong eval dependencies
uv sync --extra eval-babyai         # + BabyAI eval environment
uv sync --extra eval-appworld       # + AppWorld eval environment
uv sync --extra eval-ultrahorizon   # + UltraHorizon eval environment

# Development
uv sync --group test                # pytest + pytest-mock
uv sync --group dev                 # test group + pre-commit
```

### With conda/pip

```bash
conda create -n jaz python=3.12 && conda activate jaz

pip install -e .                    # Core library
pip install -e ".[tracing]"         # + OpenTelemetry tracing
pip install -e ".[eval-oolong]"     # + Oolong eval dependencies
pip install -e ".[eval-babyai]"     # + BabyAI eval environment
pip install -e ".[eval-appworld]"   # + AppWorld eval environment
pip install -e ".[eval-ultrahorizon]" # + UltraHorizon eval environment

# Development (dependency groups aren't supported by pip — install manually)
pip install pytest pytest-mock      # test
...
# Alternatively, if using conda/pip:
pre-commit install
```

This runs ruff and pyright automatically on each commit.

### API Keys

JAZ uses [litellm](https://docs.litellm.ai/) to interface with LLM providers. Set your API keys via any of:
...
my_tools = Library("tools", "Custom tools", [], [my_tool])
result = invoke("Use tools.my_tool on 5", return_type=int, libraries=[my_tools])
```
...
```python
# Per-invoke override
with jaz.config_override(temperature=0.7, max_tokens=2000):
    result = jaz.invoke("Creative task", return_type=str)

# Global configuration
jaz.configure(model="openai/gpt-4", max_repl_iterations=15)
```
...
├── supervisor/     # Hook system and event orchestration
├── hooks/          # Built-in hooks (logging, tracing, workflows)
...
- **Supervisor**: Event-based hook system for extensibility
- **Hooks**: Compose effects like logging, budget control, and workflow capture
...
- `workflow_replay_demo.py` - Capture and replay workflows
- `jaeger_tracing_demo.py` - Distributed tracing setup
- `multiple_choice_hook_demo.py` - Multiple choice prompting
- `identify_workflow_hook_demo.py` - Workflow identification
